	...
	{:else}
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
			{#each data.users as user}
				<div class="border border-zinc-200 rounded-lg p-4 hover:border-zinc-300 transition-colors">
					<div class="w-12 h-12 bg-zinc-100 rounded-full flex items-center justify-center text-xl mb-3">
						{user.name?.charAt(0) || '?'}
					</div>
