...
// ============================================================================
// PRODUCTS DATA FILE (static product definitions)
// ============================================================================

function getProductsData(config: ProjectConfig): string {
	return `// Product and variant types for the store
...
export const products: Product[] = [
	{
		...
		variants: [
			...
			{ id: 'classic-tee-s-white', size: 'S', color: 'White', colorHex: '#ffffff', printfulSyncVariantId: '', inStock: true },
			{ id: 'classic-tee-m-white', size: 'M', color: 'White', colorHex: '#ffffff', printfulSyncVariantId: '', inStock: true },
			{ id: 'classic-tee-l-white', size: 'L', color: 'White', colorHex: '#ffffff', printfulSyncVariantId: '', inStock: true },
			{ id: 'classic-tee-xl-white', size: 'XL', color: 'White', colorHex: '#ffffff', printfulSyncVariantId: '', inStock: true },
			{ id: 'classic-tee-s-black', size: 'S', color: 'Black', colorHex: '#1a1a1a', printfulSyncVariantId: '', inStock: true },
			{ id: 'classic-tee-m-black', size: 'M', color: 'Black', colorHex: '#1a1a1a', printfulSyncVariantId: '', inStock: true },
			{ id: 'classic-tee-l-black', size: 'L', color: 'Black', colorHex: '#1a1a1a', printfulSyncVariantId: '', inStock: true },
			{ id: 'classic-tee-xl-black', size: 'XL', color: 'Black', colorHex: '#1a1a1a', printfulSyncVariantId: '', inStock: true },
		]
	},
	{
		id: 'premium-hoodie',
		...
		category: 'hoodie',
		variants: [
			{ id: 'premium-hoodie-s-white', size: 'S', color: 'White', colorHex: '#ffffff', printfulSyncVariantId: '', inStock: true },
			{ id: 'premium-hoodie-m-white', size: 'M', color: 'White', colorHex: '#ffffff', printfulSyncVariantId: '', inStock: true },
			{ id: 'premium-hoodie-l-white', size: 'L', color: 'White', colorHex: '#ffffff', printfulSyncVariantId: '', inStock: true },
			{ id: 'premium-hoodie-xl-white', size: 'XL', color: 'White', colorHex: '#ffffff', printfulSyncVariantId: '', inStock: true },
		]
	}
];
...
function getStripeHelper(): string {
...
function getPrintfulClient(): string {
	return `// Printful API integration
...
export class PrintfulClient {
	...
	async createOrder(
		...
		const orderData: {
			...
			recipient: PrintfulRecipient;
			items: PrintfulOrderItem[];
			retail_costs?: { subtotal: string; shipping: string; total: string };
		} = {
			external_id: orderId,
			recipient,
			items
...
// ============================================================================
// DATABASE SCHEMA (orders only - products are static)
// ============================================================================

function getOrdersSchema(): string {
	return `// Store orders
...
function getStoreListingPage(config: ProjectConfig): string {
...
<svelte:head>
	<title>Store - ${config.projectName}</title>
...
<div class="max-w-6xl mx-auto px-6 py-16">
	...
	{#if products.length === 0}
		<div class="text-center py-16">
			...
			<p class="text-muted-foreground">Our store is being set up. Check back soon!</p>
		</div>
	{/if}
</div>
...
function getProductDetailPage(config: ProjectConfig): string {
	return `<script lang="ts">
...
<svelte:head>
	<title>{product.name} - ${config.projectName} Store</title>
...
function getCheckoutApi(): string {
...
function getStripeWebhook(): string {
	return `import { error, text } from '@sveltejs/kit';
...
// ============================================================================
// ORDER CONFIRMATION PAGE SERVER
// ============================================================================

function getOrderConfirmationServer(): string {
...
function getOrderConfirmationPage(config: ProjectConfig): string {
	return `<script lang="ts">
	import { Card } from '$lib/components/ui';
...
<svelte:head>
	<title>Order Confirmed - ${config.projectName} Store</title>
</svelte:head>
...
function getStoreEnvExample(): string {
...
export const storeModule: FeatureModule = {
	...
	async apply(config: ProjectConfig, outputDir: string) {
		if (config.siteType !== 'ssr-site') {
			console.log('  → Store module requires SSR site type');
			console.log('  → Skipping store module for static sites');
			return;
		}

		console.log('  → Generating store module (Stripe + Printful)');
		...
		await mkdir(join(outputDir, 'src', 'routes', 'store', 'order-confirmation'), { recursive: true });
		await mkdir(join(outputDir, 'src', 'routes', 'api', 'store', 'checkout'), { recursive: true });
		await mkdir(join(outputDir, 'src', 'routes', 'api', 'stripe', 'webhook'), { recursive: true });
		await mkdir(join(outputDir, 'static', 'products'), { recursive: true });

		// Write products data
		await writeFile(join(outputDir, 'src', 'lib', 'data', 'products.ts'), getProductsData(config));

		// Write server helpers
		await writeFile(join(outputDir, 'src', 'lib', 'server', 'stripe.ts'), getStripeHelper());
		await writeFile(join(outputDir, 'src', 'lib', 'server', 'printful.ts'), getPrintfulClient());

		// Update database schema with orders
		...
		try {
			...
			// Schema doesn't exist yet - auth module should create it
			console.log('  → Note: Orders schema requires auth module to be installed first');
		}

		// Write store routes
		await writeFile(join(outputDir, 'src', 'routes', 'store', '+page.svelte'), getStoreListingPage(config));
		await writeFile(join(outputDir, 'src', 'routes', 'store', '[slug]', '+page.server.ts'), getProductDetailServer());
		await writeFile(join(outputDir, 'src', 'routes', 'store', '[slug]', '+page.svelte'), getProductDetailPage(config));
		await writeFile(join(outputDir, 'src', 'routes', 'store', 'order-confirmation', '+page.server.ts'), getOrderConfirmationServer());
		await writeFile(join(outputDir, 'src', 'routes', 'store', 'order-confirmation', '+page.svelte'), getOrderConfirmationPage(config));

		// Write API routes
		await writeFile(join(outputDir, 'src', 'routes', 'api', 'store', 'checkout', '+server.ts'), getCheckoutApi());
		await writeFile(join(outputDir, 'src', 'routes', 'api', 'stripe', 'webhook', '+server.ts'), getStripeWebhook());
		...
		try {
			...
			await writeFile(envExamplePath, getStoreEnvExample());
		}

		// Update package.json with store dependencies
		...
		console.log('  → Pages: /store, /store/[slug], /store/order-confirmation');
		console.log('  → API: /api/store/checkout, /api/stripe/webhook');
		console.log('  → Configure STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET in .env');
		console.log('  → Optional: Configure PRINTFUL_API_KEY for automatic fulfillment');
		console.log('  → Edit src/lib/data/products.ts to add your products');
	}
};
