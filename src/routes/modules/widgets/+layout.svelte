	...
	{#if data.user?.admin}
		<div class="max-w-6xl mx-auto w-full px-6 mt-4">
			<div class="flex items-center gap-2 text-sm">
				<span class="text-muted-foreground">View mode:</span>
				<a
					href={$page.url.pathname.replace('/admin', '')}
					class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-colors {!data.isAdminView ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted'}"
				>
