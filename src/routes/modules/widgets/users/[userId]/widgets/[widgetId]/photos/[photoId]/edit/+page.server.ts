...
export const load: PageServerLoad = async ({ params, cookies, platform }) => {
	...
	if (!canEdit) redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/photos/${photoId}`);

	return { widget, photo, targetUser };
};
	...
	default: async ({ request, params, cookies, platform }) => {
		...
		try {
			await db.update(demoPhotos).set({ caption: caption?.trim() || null, updatedAt: new Date() })
				.where(and(eq(demoPhotos.id, photoId), eq(demoPhotos.widgetId, widgetId)));
			redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/photos/${photoId}`);
		} catch {
