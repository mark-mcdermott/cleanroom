...
<svelte:head><title>Add Photo - {data.thingy.name}</title></svelte:head>
...
<div class="max-w-xl mx-auto px-6 py-16">
	...
	<div class="border border-border rounded-lg p-6 bg-card">
		...
		<form method="POST" enctype="multipart/form-data" use:enhance={() => {
			...
			return async ({ result }) => {
				uploading = false;
				if (result.type === 'redirect') { toast.success('Photo added!'); goto(result.location); }
			...
			<div class="space-y-2">
				...
				<Textarea.Root id="caption" name="caption" rows={3} />
			</div>
