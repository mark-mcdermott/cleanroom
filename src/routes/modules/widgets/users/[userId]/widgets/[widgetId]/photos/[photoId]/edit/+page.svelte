...
<svelte:head>
	<title>Edit Photo - {data.widget.name}</title>
</svelte:head>

<div class="max-w-xl mx-auto px-6 py-16">
	<a href="/modules/widgets/users/{data.targetUser.id}/widgets/{data.widget.id}/photos/{data.photo.id}"
		...
		<form method="POST" use:enhance={() => {
			return async ({ result }) => {
				if (result.type === 'redirect') {
					...
					goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/photos/${data.photo.id}`);
			...
			<div class="space-y-2">
				...
				<Textarea.Root id="caption" name="caption" value={data.photo.caption || ''} placeholder="Add a caption..." rows={3} />
			</div>

			<div class="flex gap-3 pt-4">
				<Button.Root type="submit">Save Changes</Button.Root>
				<Button.Root type="button" variant="outline" onclick={() => goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/photos/${data.photo.id}`)}>Cancel</Button.Root>
			</div>
		</form>
	</div>
</div>
