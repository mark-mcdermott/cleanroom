<script lang="ts">
	...
	import { ArrowLeft, Pencil, Trash2, Plus, Image, StickyNote, Layers, Images } from 'lucide-svelte';
	import { toast } from 'svelte-sonner';

	let { data } = $props();

	function handleDelete() {
		if (confirm(`Are you sure you want to delete "${data.widget.name}"? This will also delete all thingys, notes, photos, and galleries.`)) {
...
<svelte:head>
	<title>{data.widget.name} - Widgets</title>
	<meta name="description" content={data.widget.description || 'View widget details'} />
</svelte:head>
...
<div class="max-w-4xl mx-auto px-6 py-16">
	...
	<div class="bg-white border border-zinc-200 rounded-lg p-6 mb-6">
		<div class="flex items-start justify-between mb-4">
			...
			{#if data.canEdit}
				<div class="flex gap-2">
					<Button.Root variant="outline" class="cursor-pointer" onclick={() => goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/edit`)}>
		...
		{#if data.currentUser?.admin}
			<div class="text-sm text-zinc-500 border-t border-zinc-200 pt-4 mt-4 space-y-1">
				...
				<div class="flex justify-between">
					<span>Created</span>
					<span>{data.widget.createdAt ? new Date(data.widget.createdAt).toLocaleString() : 'N/A'}</span>
				</div>
				<div class="flex justify-between">
					<span>Updated</span>
					<span>{data.widget.updatedAt ? new Date(data.widget.updatedAt).toLocaleString() : 'N/A'}</span>
				</div>
			</div>
		{/if}
	</div>
	...
	<div class="bg-white border border-zinc-200 rounded-lg p-6 mb-6">
		<div class="flex items-center justify-between mb-4">
			...
			{#if data.canEdit}
				<Button.Root variant="outline" size="sm" class="cursor-pointer" onclick={() => goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/thingys/new`)}>
	...
	<div class="bg-white border border-zinc-200 rounded-lg p-6 mb-6">
		<div class="flex items-center justify-between mb-4">
			...
			{#if data.canEdit}
				<Button.Root variant="outline" size="sm" class="cursor-pointer" onclick={() => goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/notes/new`)}>
					<Plus class="w-4 h-4 mr-1" />
					Add Note
				</Button.Root>
			{/if}
	...
	<div class="bg-white border border-zinc-200 rounded-lg p-6 mb-6">
		<div class="flex items-center justify-between mb-4">
			...
			{#if data.canEdit}
				<Button.Root variant="outline" size="sm" class="cursor-pointer" onclick={() => goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/photos/new`)}>
					<Plus class="w-4 h-4 mr-1" />
					Add Photo
				</Button.Root>
			{/if}
	...
	use:enhance={() => {
		return async ({ result }) => {
			if (result.type === 'redirect') {
				...
				goto(`/modules/widgets/users/${data.targetUser.id}/widgets`);
