...
<svelte:head>
	<title>Note - {data.widget.name}</title>
</svelte:head>
		...
		<div class="flex items-start justify-between mb-4">
			...
			{#if data.canEdit}
				<div class="flex gap-2">
					<Button.Root variant="outline" size="sm" class="cursor-pointer" onclick={() => goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/notes/${data.note.id}/edit`)}>
