...
export const load: PageServerLoad = async ({ params, cookies, platform }) => {
	...
	return { widget, thingy, note, targetUser, canEdit: currentUser?.id === userId || currentUser?.admin };
};
...
export const actions: Actions = {
	delete: async ({ params, cookies, platform }) => {
		...
		try {
			await db.delete(demoNotes).where(and(eq(demoNotes.id, noteId), eq(demoNotes.thingyId, thingyId)));
