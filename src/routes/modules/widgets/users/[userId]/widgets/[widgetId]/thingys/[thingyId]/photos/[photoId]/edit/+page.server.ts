...
export const load: PageServerLoad = async ({ params, cookies, platform }) => {
	...
	if (!canEdit) redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/thingys/${thingyId}/photos/${photoId}`);

	return { widget, thingy, photo, targetUser };
};
...
export const actions: Actions = {
	default: async ({ request, params, cookies, platform }) => {
		...
		try {
			await db.update(demoPhotos).set({ caption: caption?.trim() || null, updatedAt: new Date() })
				.where(and(eq(demoPhotos.id, photoId), eq(demoPhotos.thingyId, thingyId)));
			redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/thingys/${thingyId}/photos/${photoId}`);
		} catch {
