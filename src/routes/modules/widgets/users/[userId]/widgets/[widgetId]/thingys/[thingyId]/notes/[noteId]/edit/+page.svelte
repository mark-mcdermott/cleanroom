...
<svelte:head>
	<title>Edit Note - {data.thingy.name}</title>
</svelte:head>

<div class="max-w-xl mx-auto px-6 py-16">
	<a href="/modules/widgets/users/{data.targetUser.id}/widgets/{data.widget.id}/thingys/{data.thingy.id}/notes/{data.note.id}"
		...
		<form method="POST" use:enhance={() => {
			return async ({ result }) => {
				if (result.type === 'redirect') {
					...
					goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/thingys/${data.thingy.id}/notes/${data.note.id}`);
			...
			<div class="flex gap-3 pt-4">
				...
				<Button.Root type="button" variant="outline" onclick={() => goto(`/modules/widgets/users/${data.targetUser.id}/widgets/${data.widget.id}/thingys/${data.thingy.id}/notes/${data.note.id}`)}>Cancel</Button.Root>
			</div>
		</form>
	</div>
</div>
