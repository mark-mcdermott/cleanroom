...
export const load: PageServerLoad = async ({ params, cookies, platform }) => {
	const { userId, widgetId, thingyId, noteId } = params;
	...
	const [note] = await db.select().from(demoNotes)
		.where(and(eq(demoNotes.id, noteId), eq(demoNotes.thingyId, thingyId)));
	if (!note) redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/thingys/${thingyId}`);
	...
	if (!canEdit) redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/thingys/${thingyId}/notes/${noteId}`);

	return { widget, thingy, note, targetUser };
};
...
export const actions: Actions = {
	default: async ({ request, params, cookies, platform }) => {
		...
		try {
			await db.update(demoNotes).set({ content: content.trim(), updatedAt: new Date() })
				.where(and(eq(demoNotes.id, noteId), eq(demoNotes.thingyId, thingyId)));
			redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/thingys/${thingyId}/notes/${noteId}`);
		} catch {
