...
export const load: PageServerLoad = async ({ params, cookies, platform }) => {
	const { userId, widgetId, noteId } = params;
	...
	const [note] = await db.select().from(demoNotes)
		.where(and(eq(demoNotes.id, noteId), eq(demoNotes.widgetId, widgetId)));
	if (!note) redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}`);
	...
	if (!canEdit) redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/notes/${noteId}`);

	return { widget, note, targetUser };
};
	...
	default: async ({ request, params, cookies, platform }) => {
		...
		try {
			await db.update(demoNotes).set({ content: content.trim(), updatedAt: new Date() })
				.where(and(eq(demoNotes.id, noteId), eq(demoNotes.widgetId, widgetId)));
			redirect(302, `/modules/widgets/users/${userId}/widgets/${widgetId}/notes/${noteId}`);
		} catch {
