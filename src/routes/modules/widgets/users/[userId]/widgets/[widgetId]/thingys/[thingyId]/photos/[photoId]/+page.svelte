	...
	<div class="bg-card border border-border rounded-lg overflow-hidden">
		...
		<div class="p-6">
			<div class="flex items-start justify-between">
				<div>
					{#if data.photo.caption}<p class="text-muted-foreground">{data.photo.caption}</p>
					{:else}<p class="text-muted-foreground italic">No caption</p>{/if}
					<p class="text-sm text-muted-foreground mt-2">{data.photo.filename || 'Unknown'}</p>
				</div>
				{#if data.canEdit}
