...
- `coding-patterns.md` - Service objects, queries, coding patterns, etc.
...
- `javascript.md` - JavaScript/TypeScript standards (`JS.*` codes)
...
- **Context vs Rules**:
  - Use prose paragraphs for contextual information (e.g., "This app uses Svelte...")
...
Codex CLI users can "install" slash commands via symlink:
    ...
    ln -sfn $PWD/.llm/commands ~/.codex/prompts
...
- `qtest.md` - Test writing (JavaScript)
...
- `dan-abramov.md` - Dan Abramov-style agent command
...
- `dan-abramov.md` - Dan Abramov-style agent
...
### Claude Code
- **Entry Point**: `/CLAUDE.md` (symlink to `.llm/root-context.md`)
- **Syntax**: Uses `@` syntax to import files (e.g., `@.llm/rules/javascript.md`)
- **Loading**: Files referenced in `CLAUDE.md` are loaded automatically

### GitHub Copilot
- **Entry Point**: `/AGENTS.md` (symlink to `.llm/root-context.md`)
- **Syntax**: Uses `@` syntax to import files
- **Loading**: Depends on Copilot's implementation
- **Commands**: Read from `.llm/command-index.md` when a command is invoked
...
```bash
ln -s .llm/root-context.md CLAUDE.md
ln -s .llm/root-context.md AGENTS.md
```
...
3. Must NOT reference any other `@.llm/` files (leaf nodes in DAG)
