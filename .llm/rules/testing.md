...
## Purpose
Defines testing standards, quality criteria, and best practices for JavaScript/TypeScript tests.
...
- **TEST.DB-2 (SHOULD)**: Trust database to enforce DB-level constraints defined in Drizzle schema
- **TEST.DB-3 (SHOULD)**: Test domain validations (presence, format) not DB constraints (uniqueness, length limits)
...
- **TEST.MOCK-3 (SHOULD)**: Mock Drizzle client for isolated unit tests
...
### Local Testing
- **TEST.FLOW-1 (MUST)**: Run all tests locally before pushing to `staging`
...
- **TEST.FLOW-5 (MUST)**: Verify tests pass before every push to `staging`

## TanStack Query Testing

### Query Testing
- **TEST.QUERY-1 (SHOULD)**: Use MSW to mock API responses for queries
- **TEST.QUERY-2 (SHOULD)**: Test loading and error states
- **TEST.QUERY-3 (SHOULD)**: Disable retry in tests (`retry: false`)

### Mutation Testing
- **TEST.QUERY-4 (SHOULD)**: Test mutation success and failure paths
- **TEST.QUERY-5 (SHOULD)**: Verify query invalidation after mutations
